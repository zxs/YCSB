...
public class DynamoDBClient extends DB {
	...
	private static Logger logger = LoggerFactory.getLogger(DynamoDBClient.class);
...
//			logger.setLevel(Level.DEBUG);
		...
		String endpoint = getProperties()
				.getProperty("dynamodb.endpoint", null);
		String credentialsFile = getProperties().getProperty(
				"dynamodb.awsCredentialsFile", null);
		String primaryKey = getProperties().getProperty("dynamodb.primaryKey",
				null);
		String consistentReads = getProperties().getProperty(
				"dynamodb.consistentReads", null);
		String connectMax = getProperties().getProperty("dynamodb.connectMax",
				null);
		...
		try {
			...
			String errMsg = "DynamoDBClient.init(): Could not initialize DynamoDB client: "
					+ e1.getMessage();
			logger.error(errMsg);
		}
	}
	...
	public int scan(String table, String startkey, int recordcount,
		 ...
		 */
		GetItemRequest greq = new GetItemRequest(table,
				createPrimaryKey(startkey));
	...
	public int update(String table, String key,
		...
		UpdateItemRequest req = new UpdateItemRequest(table,
				createPrimaryKey(key), attributes);
	...
	public int insert(String table, String key,
			HashMap<String, ByteIterator> values) {
		logger.debug("insertkey: " + primaryKeyName + "-" + key
				+ " from table: " + table);
	...
	public int delete(String table, String key) {
		...
		DeleteItemRequest req = new DeleteItemRequest(table,
	...
	private HashMap<String, ByteIterator> extractResult(
		...
		if (null == item)
			return null;
		HashMap<String, ByteIterator> rItems = new HashMap<String, ByteIterator>(
				item.size());

		for (Entry<String, AttributeValue> attr : item.entrySet()) {
			logger.debug(String.format("Result- key: %s, value: %s",
					attr.getKey(), attr.getValue()));
			rItems.put(attr.getKey(), new StringByteIterator(attr.getValue()
					.getS()));
		}
		return rItems;
	}
