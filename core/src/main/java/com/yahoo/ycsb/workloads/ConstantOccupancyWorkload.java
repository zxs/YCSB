 ...
 * 'https://database.cs.wisc.edu/cidr/cidr2007/papers/cidr07p34.pdf'>Fragmentation
 * in Large Object Repositories</a>, CIDR 2006. [<a href=
 * 'https://database.cs.wisc.edu/cidr/cidr2007/slides/p34-sears.ppt'>Presentation</a>
 * ]
 * </p>
 * 
 * 
...
public class ConstantOccupancyWorkload extends CoreWorkload {
	...
	public void init(Properties p) throws WorkloadException {
		disksize = Long.parseLong(p.getProperty(DISK_SIZE_PROPERTY,
				DISK_SIZE_PROPERTY_DEFAULT + ""));
		storageages = Long.parseLong(p.getProperty(STORAGE_AGE_PROPERTY,
				STORAGE_AGE_PROPERTY_DEFAULT + ""));
		occupancy = Double.parseDouble(p.getProperty(OCCUPANCY_PROPERTY,
		...
		if (p.getProperty(Client.RECORD_COUNT_PROPERTY) != null
			...
			System.err
					.println("Warning: record, insert or operation count was set prior to initting ConstantOccupancyWorkload.  Overriding old values.");
		}
		IntegerGenerator g = CoreWorkload.getFieldLengthGenerator(p);
		double fieldsize = g.mean();
		int fieldcount = Integer.parseInt(p.getProperty(FIELD_COUNT_PROPERTY,
