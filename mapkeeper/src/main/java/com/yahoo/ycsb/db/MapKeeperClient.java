...
public class MapKeeperClient extends DB {
	...
	private synchronized static void initDB(Properties p, MapKeeper.Client c)
			throws TException {
		if (!initteddb) {
			initteddb = true;
			c.addMap(p.getProperty(CoreWorkload.TABLENAME_PROPERTY,
	...
	public void init() {
		...
		writeallfields = Boolean.parseBoolean(getProperties().getProperty(
				CoreWorkload.WRITE_ALL_FIELDS_PROPERTY,
	...
	ByteBuffer encode(HashMap<String, ByteIterator> values) {
		...
		for (String k : values.keySet()) {
			...
			array[i] = '\t'; // XXX would like to use sane delimiter (null, 254,
								// 255, ...) but java makes this nearly
								// impossible
			i++;
	...
	void decode(Set<String> fields, String tups,
			HashMap<String, ByteIterator> tup) {
		...
		for (int i = 0; i < tok.length; i += 2) {
			if (fields == null || fields.contains(tok[i])) {
				if (tok.length < i + 2) {
					throw new IllegalStateException("Couldn't parse tuple <"
							+ tups + "> at index " + i);
				}
				if (tok[i] == null || tok[i + 1] == null)
					throw new NullPointerException("Key is " + tok[i]
							+ " val is + " + tok[i + 1]);
	...
	public int read(String table, String key, Set<String> fields,
		...
		try {
			...
			int ret = ycsbThriftRet(succ, ResponseCode.RecordExists,
	...
	public int scan(String table, String startkey, int recordcount,
		...
		try {
			...
			RecordListResponse res = c.scan(table, ScanOrder.Ascending,
					bufStr(startkey), true, null, false, recordcount, 0);
			int ret = ycsbThriftRet(res.responseCode, ResponseCode.Success,
					ResponseCode.ScanEnded);
			if (ret == 0) {
				for (Record r : res.records) {
					HashMap<String, ByteIterator> tuple = new HashMap<String, ByteIterator>();
					// Note: r.getKey() and r.getValue() call special helper
					// methods that trim the buffer
					// to an appropriate length, and memcpy it to a byte[].
					// Trying to manipulate the ByteBuffer
					// directly leads to trouble.
					tuple.put("key",
							new StringByteIterator(new String(r.getKey())));
					decode(fields, new String(r.getValue())/*
															 * strBuf(r.
															 * bufferForValue())
															 */, tuple);
					result.add(tuple);
	...
	public int update(String table, String key,
		...
		try {
			...
			return ycsbThriftRet(succ, ResponseCode.RecordExists,
					ResponseCode.RecordNotFound);
		} catch (TException e) {
			e.printStackTrace();
			return 2;
	...
	public int insert(String table, String key,
		...
		try {
			int ret = ycsbThriftRet(
					c.insert(table, bufStr(key), encode(values)),
	...
	public int delete(String table, String key) {
		try {
			return ycsbThriftRet(c.remove(table, bufStr(key)),
					ResponseCode.Success, ResponseCode.RecordExists);
		} catch (TException e) {
			e.printStackTrace();
			return 2;
