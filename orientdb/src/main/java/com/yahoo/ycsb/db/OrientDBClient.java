...
public class OrientDBClient extends DB {
	...
	public void init() throws DBException {
		...
		try {
			...
			if (db.exists()) {
				...
				System.out
						.println("OrientDB database not found, create fresh db");
				db.create();
			}
	...
	public int scan(String table, String startkey, int recordcount,
		...
		try {
			final Collection<ODocument> documents = dictionary.getIndex()
					.getEntriesMajor(startkey, true, recordcount);
			for (ODocument document : documents) {
				...
				for (String field : fields)
					entry.put(
							field,
							new StringByteIterator((String) document
									.field(field)));
			}
