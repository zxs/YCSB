...
public class ElasticSearchClient extends DB {
	...
	public void init() throws DBException {
		...
		Boolean newdb = Boolean.parseBoolean(props.getProperty(
				"elasticsearch.newdb", "false"));
		Builder settings = settingsBuilder()
				.put("node.local", "true")
				.put("path.data",
						System.getProperty("java.io.tmpdir") + "/esdata")
		...
		settings.put(props);
		System.out.println("ElasticSearch starting node = "
				+ settings.get("cluster.name"));
		System.out.println("ElasticSearch node data path = "
		...
		if (newdb) {
			client.admin().indices().prepareDelete(indexKey).execute()
					.actionGet();
			client.admin().indices().prepareCreate(indexKey).execute()
					.actionGet();
		} else {
			boolean exists = client.admin().indices()
					.exists(Requests.indicesExistsRequest(indexKey))
					.actionGet().isExists();
			if (!exists) {
				client.admin().indices().prepareCreate(indexKey).execute()
						.actionGet();
	...
	public int read(String table, String key, Set<String> fields,
		...
		try {
			final GetResponse response = client
					.prepareGet(indexKey, table, key).execute().actionGet();
	...
	public int scan(String table, String startkey, int recordcount,
		...
		try {
			...
			final SearchResponse response = client.prepareSearch(indexKey)
					.setTypes(table).setQuery(matchAllQuery())
					.setFilter(filter).setSize(recordcount).execute()
					.actionGet();
			...
			for (SearchHit hit : response.getHits()) {
				...
				for (String field : fields) {
					entry.put(field, new StringByteIterator((String) hit
							.getSource().get(field)));
				}
